...
# --- Setup ---
st.set_page_config(page_title="Pulse – AI Feedback Tool", layout="wide")

# --- Branding ---
st.image("Logo-Hello AI.jpg", width=150)
st.title("🔵 Pulse – AI-Powered Feedback & Marking Tool")
st.markdown("""
Welcome to **Pulse** – an AI-powered feedback and marking tool designed to accelerate academic assessment.

_Powered by OpenAI and aligned with the **Hello AI** initiative to integrate ethical and intelligent automation into education._
...
st.student_name = st.text_input("Student Name")
...
if st.button("Generate AI Feedback"):
    if student_submission and rubric_df is not None and st.student_name:
        with st.spinner("Reading and analysing the student work..."):
            ...
            if st.button("Download as CSV"):
                ...
                for criterion, score in scores:
                    writer.writerow([st.student_name, criterion, score])
                st.download_button(
                    label="Download Scores CSV",
                    data=output.getvalue(),
                    file_name=f"{st.student_name}_scores.csv",
                    mime="text/csv"
                )

            if st.button("Download Feedback as DOCX"):
                doc = Document()
                doc.add_heading(f"Assessment Feedback for {st.student_name}", level=1)
                ...
                st.download_button(
                    ...
                    file_name=f"{st.student_name}_feedback.docx",
